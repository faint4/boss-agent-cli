...
- **招聘者完整链路**：`hr journey` 与本地 Web 共用应用合同，单条回复必须“准备→确认”；候选人搜索、简历、聊天和职位管理等旧命令继续兼容
...
boss hr jobs list
boss hr journey --input-json '{"action":"run","steps":[{"action":"openings"},{"action":"select-opening","reference":"$first"},{"action":"applicants"},{"action":"inspect","reference":"$first"},{"action":"prepare-reply","reference":"$first","message":"感谢投递"},{"action":"confirm","intent_id":"$pending"}]}'
```
...
`boss schema` 暴露 40 个顶层命令 + 10 个一级招聘者子命令，按工作流分组：
...
- **招聘者**：`hr journey`（Web/CLI/MCP 共享合同，显式确认后单次回复）· 兼容 `hr applications/candidates/resume/chat/chatmsg/last-messages/reply/request-resume/jobs`
